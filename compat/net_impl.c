...
static avs_error_t get_mtu(net_socket_impl_t *net_socket, int *out_mtu) {
    if (net_socket->forced_mtu > 0) {
        *out_mtu = net_socket->forced_mtu;
    } else {
        *out_mtu = 1464;
    }
    return AVS_OK;
...
static avs_error_t get_udp_overhead(net_socket_impl_t *net_socket, int *out) {
    switch (net_socket->address_family) {
#ifdef CONFIG_NET_IPV4
    case AF_INET:
...
static avs_error_t get_inner_mtu(net_socket_impl_t *net_socket, int *out_mtu) {
    if (net_socket->socktype != SOCK_DGRAM) {
        LOG(ERROR,
            _("get_opt_net: inner MTU calculation unimplemented for TCP"));
        return avs_errno(AVS_ENOTSUP);
    }
    (void) get_mtu(net_socket, out_mtu);
    int udp_overhead;
    avs_error_t err;
...
static avs_error_t net_get_opt(avs_net_socket_t *sock_,
    ...
    case AVS_NET_SOCKET_OPT_MTU:
        return get_mtu(sock, &out_option_value->mtu);
    case AVS_NET_SOCKET_OPT_INNER_MTU:
        return get_inner_mtu(sock, &out_option_value->mtu);
